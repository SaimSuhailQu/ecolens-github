...
export const analyzeRegionWithGEE = async (
  ...
  const primaryIndexId: string = analysisCategory === 'All' ? 'ndvi' : 
                         analysisCategory === 'Heat' ? 'uhi' :
                         analysisCategory === 'Water' ? 'ndwi' :
                         analysisCategory === 'Burn' ? 'nbr' :
                         analysisCategory === 'Urban' ? 'ndbi' :
                         analysisCategory === 'Geological' ? 'alui' :
                         analysisCategory === 'Climate' ? 'pdsi' : 'ndvi';
...
${primaryIndexId === 'uhi' ? `// Rural mask for UHI baseline
var dw = ee.ImageCollection('GOOGLE/DYNAMICWORLD/V1')
  .filterDate('${startDate}', '${endDate}')
  .filterBounds(geometry)
  .select('label').mode();
var ruralMask = dw.eq(1).or(dw.eq(2)).or(dw.eq(4)).or(dw.eq(5));\n` : ''}
${getGEEScriptSnippet(primaryIndexId, resolution)}

Map.centerObject(geometry, 12);
Map.addLayer(result, {
  min: ${primaryIndexId === 'uhi' ? 0 : primaryIndexId === 'lst' ? 10 : -1},
  max: ${primaryIndexId === 'uhi' ? 10 : primaryIndexId === 'lst' ? 50 : 1},
  palette: ${JSON.stringify(
    primaryIndexId === 'lst' || primaryIndexId === 'uhi' ? ['#0d0887', '#5c01a6', '#9c179e', '#cc4678', '#ed7953', '#fdb32f', '#f0f921'] : 
    ['#a50026', '#d73027', '#f46d43', '#fdae61', '#fee08b', '#ffffbf', '#d9ef8b', '#a6d96a', '#66bd63', '#1a9850', '#006837']
  )}
}, '${primaryIndexId.toUpperCase()}');`;
